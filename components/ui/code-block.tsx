...
function CodeBlockPane({
  ...
  style,
  highlightLines,
  highlightClassName = 'bg-primary/10 border-l-2 border-primary text-foreground',
  ...
  return (
    ...
    >
      <ScrollArea className="max-h-43.75 *:data-[slot=scroll-area-viewport]:h-auto! *:data-[slot=scroll-area-viewport]:max-h-43.75">
        {showCopy && <CodeBlockCopyButton code={code} className="absolute right-2 top-2 z-10" />}
        ...
        {html ? (
          useLineView ? (
                  ...
                  {lines.map((line, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className={cn(
                          'flex items-stretch border-l-2 border-transparent px-4 py-[0.5px]',
                          isHighlighted && highlightClassName
                        )}
                      >
                        {showLineNumbers && (
                          <span className="mr-4 w-6 shrink-0 select-none text-right font-mono text-xs leading-relaxed text-muted-foreground/60">
                            {lineNumber}
                          </span>
                        )}
            ...
            <>
              <div
                className={cn(
                  '[&>pre]:whitespace-pre [&>pre]:bg-transparent!',
                  '[&>pre]:p-4 [&>pre]:font-mono [&>pre]:text-sm',
                  '[&>pre]:leading-relaxed'
                )}
  ...
  return (
    ...
    >
      <div
        ...
        className="flex items-center justify-between gap-2 border-b border-border bg-muted/30"
      >
        {isMulti ? (
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1">
            <TabsList variant="line" className="h-auto rounded-none bg-transparent">
              {normalizedFiles.map((file) => (
                <TabsTrigger
                  key={file.filename}
                  value={file.filename}
                  className={cn(
                    'h-auto px-3 py-2 font-mono text-xs font-medium',
